...
//------------------------------------------------------------------------------
bool __fastcall TContacts::LoadFromDB (TADOQuery *query, String &strErr)
{
	bool f;
	String strSql;
	TContactInfo contact;

	clear ();
	strSql = "select * from " + TContactInfo::Table + ";";
	if ((f = RunQuery (query, strSql, QueryOpen, strErr)) == true) {
		query->First ();
		while (!query->Eof) {
			if (contact.LoadFromDB (query)) {
				push_back (contact);
			}
			query->Next ();
		}
...
//------------------------------------------------------------------------------
bool __fastcall TContacts::LoadFromDB (TADOConnection *db, String &strErr)
{
	TADOQuery *query = InitQuery (db, NULL);
	bool f;
